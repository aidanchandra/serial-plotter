...
class FakeMessage:
    def __init__(self, func1, func2, func3) -> None:
        self.func1 = func1
        self.func2 = func2
        self.func3 = func3
    ...
    def process(self):
        ...
        self.datas = [
            ("A", self.func1(self.timestamp)),
            ("B", self.func2(self.timestamp)),
            ("C", self.func3(self.timestamp)),
        ]
        self.statuses = [
            ("!Random 1", str(bool(random.randint(0, 1)))),
            ("!Random 2", str(bool(random.randint(0, 1)))),
        ]

        self.data = {
            "timestamp": self.timestamp,
            "local_t": self.timestamp,
        ...
        if port[0] == "Example Data":
            ...
            while misc_data["on"]:
                ...
                if len(send_messages) > 0:
                    ...
                    send_messages[:] = []
                # Messages to recieve
                if time.perf_counter_ns() - start_time > RATE:
                    fake_message = FakeMessage(np.sin, np.cos, np.tan)
                    fake_message.process()
                    processed = fake_message.get()
                    read_messages.append(processed)
                    self.messages.append(fake_message.json_friendly_object())
            ...
            while misc_data["on"]:
                ...
                if ellapsed_time > 5:
                    if self.log_enabled:
                        ...
                        log_time_ellapsed = time.perf_counter_ns() - log_start_time
                        logging.debug(
                            "Logging completed in " + str(log_time_ellapsed) + "nS"
                        )
